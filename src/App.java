...
public class App {
    ...
    */
    ...
    static AVL<Integer, Cliente> lerClientes(String nomeArquivoDados) {
        ...
        try{
            caminho = new File(nomeArquivoDados);
        ...
        if (arquivo.hasNext()) {
            qtdLinhas = Integer.parseInt(arquivo.nextLine()) ;
        }

        if (qtdLinhas > 0) {
            for (int i = 0; i < qtdLinhas; i++){
                Cliente c = new Cliente(arquivo.nextLine());
                clientesCadastrados.inserir(c.hashCode(), c);
                quantosClientes++;
            }   
        }
    ...
    public static void pedidosDoCliente() {
        ...
        encontrado = clientesPorId.pesquisar(doc);

        historico = pedidosPorCliente.pesquisar(encontrado);
